...
def transdecoder_to_dic_XCGene(file_names,no_redundant = None):
    """
    ...
    if no_redundant is None:
        return dic_XCGene
    else:
        genes2keep = set()
        for ele in SeqIO.parse(open(no_redundant),"fasta"):
            genes2keep.add(ele.id)
        dic_XCGeneNew={}
        for ele in genes2keep:
            dic_XCGeneNew[ele] = dic_XCGene[ele]
        return dic_XCGeneNew


def fileAgOGS2dic_XCGenes(peptidefile = None,transcriptfile=None):
    """
    given peptide file and transcriptfile, return dic_XCGene. Because AgOGS4.3, peptide id looks like "AGAP000005-PA", transcript id 
    looks like "AGAP000002-RA", use the peptide id here because we are using peptide sequence for blast search. 
    use transcript sequence as cds sequence
    """
    if peptidefile is None:
        peptidefile = "E:\\store_for_D\\Insects Info\\Anopheles gambiae\\Anopheles-gambiae-PEST_PEPTIDES_AgamP4.3.fa"
    if transcriptfile is None:
        transcriptfile = "F:\\Insects\\Anopheles_gambiae\\Anopheles-gambiae-PEST_TRANSCRIPTS_AgamP4.3.fa"
    fo_protein = open(peptidefile,"r")
    fo_mRNA = open(transcriptfile,"r")
    dic_protein = SeqIO.to_dict(SeqIO.parse(fo_protein,"fasta"))
    ls_transcript = list(SeqIO.parse(fo_mRNA,"fasta"))
    dic_mRNA ={}
    for mRNA in ls_transcript:
        key = mRNA.id.replace("-R","-P")
        dic_mRNA[key] = mRNA
    dic_XCGene ={}
    for name in dic_protein:
        dic_XCGene[name] = XCGene(name,dic_protein[name],dic_mRNA[name],dic_mRNA[name])
    return dic_XCGene
...
def maker_to_cuff(dic_maker2cuff_ml, dicGene_maker, dicGene_cuff, coverage = 0.7, min_length = 200):
    ...
    for elements in dic_maker2cuff_ml:
        ...
        if dic_maker2cuff_ml[elements] * (1/ (coverage * dicGene_maker[elements[0]].protein_length()) + 1/min_length) < 1:
            keepmaker.append((elements[0], "AgamP4.3, bad match in cufflinks"))
    makerset = set(makerlist)
    for elements in dicGene_maker:
        if elements not in makerset:
            keepmaker.append((elements,"AgamP4.3, no match in cufflinks"))
    return keepmaker
...
file_maker2Uni, file_cuff2Uni, file_Trinity2Uni, file_Oases2Uni, file_uniprot, fout_mcot, coverage = 0.7, min_length =200,\
 file_cuffUniqueAndNoneWithin = None, file_TrinityUN = None, file_OasesUN = None):
    ...
    mcot=[]
    mcot_to_chose = {}
    dicGene_maker = fileAgOGS2dic_XCGenes(file_maker,None)
    dicGene_cuff = transdecoder_to_dic_XCGene(file_cuff,file_cuffUniqueAndNoneWithin)
    dicGene_Trinity = transdecoder_to_dic_XCGene(file_Trinity,file_TrinityUN)
    dicGene_Oases = transdecoder_to_dic_XCGene(file_Oases,file_OasesUN)
    dicGene={}
    ...
    diclen_maker = fasta_length(open_fasta_to_list(file_maker))
    ...
    for elements in mcot_to_chose:
        if mcot_to_chose[elements] == []:
            ...
            key =mcot_to_chose[elements][1]
#            if key not in dicGene: # for some reason, T71|m.62 change to gnl|T71|m.62 in the blast output. these two line correct this bug
#                key = key[4:]
            lOases = dicGene[key].protein_length()
    ...
    if file_TrinityUN == None:
        list_GeneTrinityUN = fasta_within_seq_big_faster(open_fasta_to_list(file_Trinity+".transdecoder.pep"))
    else:
        list_GeneTrinityUN = open_fasta_to_list(file_TrinityUN)
    if file_OasesUN == None:
        list_GeneOasesUN = fasta_within_seq_big_faster(open_fasta_to_list(file_Oases+".transdecoder.pep"))
    else:
    ...
    list_mcot_protein_UandNwithin3 = fasta_within_seq_big_withError(list_mcot_protein,0.02)
    ...
    for elements in mcot_final:
        ...
        if elements[1][1] in dicGene_cuff:
            ...
            long_description = "ori: AgamP4.3 " + des_makerid + " mlen: " + des_makerlen + \
            " cuff: " + des_cuffid + " cl: " + des_cufflen + " ml_mc: " + des_ml_mc +\
            " Trinity: " + des_Trinityid + " tl: " + des_Trinitylen + " ml_mt: " + des_ml_mt +\
            " Bridger: " + des_Oasesid + " bl: " + des_Oaseslen + " ml_mb: " + des_ml_mo
            if elements[1][0] in dicGene_Trinity:
                short_description += ".MT"
            elif elements[1][0] in dicGene_Oases:
                short_description += ".MB"
            ...
            short_description += ".BB"
            long_description = "ori: Bridger " + elements[1][1] + " blen: " + str(diclen_all[elements[1][1]])
...
file_maker = folder + "Anopheles-gambiae-PEST_PEPTIDES_AgamP4.3.fa"
folderA = folder +"Assemblies\\"
file_cuff = folderA + "AgCufflinks20160310.fa"
file_Trinity  = "F:\\Insects\\Anopheles_gambiae\\TrinityNew\\TrinityConbined\\20160406TrinityCombined98.fa"
file_Oases  = folderA + "20160314BridgerAll.fa"
folderC = "F:\\Insects\\Anopheles_gambiae\\MCOT2\\Blast\\"
file_maker2cuff = folderC + "O2C.txt"
file_cuff2maker = folderC + "C2O.txt"
file_maker2Trinity = folderC + "O2T.txt"
file_Trinity2maker = folderC + "T2O.txt"
file_maker2Oases = folderC + "O2B.txt"
file_Oases2maker = folderC + "B2O.txt"
file_cuff2Trinity = folderC + "C2T.txt"
file_Trinity2cuff = folderC + "T2C.txt"
file_cuff2Oases = folderC + "C2B.txt"
file_Oases2cuff = folderC + "B2C.txt"
file_maker2Uni = folderC + "O2A.txt"
file_cuff2Uni = folderC + "C2A.txt"
file_Trinity2Uni = folderC + "T2A.txt"
file_Oases2Uni = folderC + "B2A.txt"
file_uniprot = "F:\\Insects\\uniprotkb_arthropoda"
fout_mcot = "F:\\Insects\\Anopheles_gambiae\\MCOT2\\20160407AgMcot"
coverage = 0.7
min_length =200
file_cuffUniqueAndNoneWithin = folderA + "AgCufflinks20160310.fa.transdecoder.pepnonredun"
file_TrinityUN = "F:\\Insects\\Anopheles_gambiae\\TrinityNew\\TrinityConbined\\20160406TrinityCombined98.fa.transdecoder.pepNoWithin"
file_OasesUN = file_Oases+"UN"

cuff_to_denovo(file_maker, file_cuff, file_Trinity, file_Oases, \
file_maker2cuff, file_cuff2maker, file_maker2Trinity, file_Trinity2maker, file_maker2Oases, file_Oases2maker, \
file_cuff2Trinity, file_Trinity2cuff, file_cuff2Oases, file_Oases2cuff,\
file_maker2Uni, file_cuff2Uni, file_Trinity2Uni, file_Oases2Uni, file_uniprot, fout_mcot, coverage = 0.7, min_length =200,\
 file_cuffUniqueAndNoneWithin , file_TrinityUN , file_OasesUN )
 
 #save dic_ml_denovo
fo = open("dic_ml_denovo.txt","w")
fo.write(str(dic_ml_denovo))
fo.close()

fo = open("mcot_final.txt","w")
fo.write(str(mcot_final))
fo.close()